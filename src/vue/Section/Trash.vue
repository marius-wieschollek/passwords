            ...
            <div class="item-list">
                ...
                <folder-line :folder="folder" v-for="folder in folders" :key="folder.id" v-on:restore="restoreFolderAction(folder)">
                    <template #custom-action>
                        <nc-action-button @click.stop.prevent="restoreFolderAction(folder)" data-pw-action="restore" close-after-click>
                            <template #icon>
                ...
                <tag-line :tag="tag" v-for="tag in tags" :key="tag.id" v-if="tag.trashed" v-on:restore="restoreTagAction(tag)">
                    <template #custom-action>
                        <nc-action-button @click.stop.prevent="restoreTagAction(tag)" data-pw-action="restore" close-after-click>
                            <template #icon>
                                <restore-icon :size="20"/>
                            </template>
                            {{ t('Restore') }}
                ...
                <password-line :password="password" v-for="password in passwords" v-if="password.trashed" v-on:restore="restorePasswordAction(password)" :key="password.id">
                    <template #custom-action>
                        <nc-action-button @click.stop.prevent="restorePasswordAction(password)" data-pw-action="restore" close-after-click>
                            <template #icon>
                                <restore-icon :size="20"/>
                            </template>
                            {{ t('Restore') }}
