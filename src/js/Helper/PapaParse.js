...
(function(root, factory) {
    ...
    function ChunkStreamer(config) {
        ...
        function replaceConfig(config) {
            ...
            }  // disable Range header if not streaming; bad values break IIS - see issue #196
                ...
                if(input[cursor] === quoteChar) {
                    ...
                    for(; ;) {
                        ...
                        if(quoteChar === escapeChar && input[quoteSearch + 1] === escapeChar) {
                            if(config.badQuotes &&
                               (
                                   input.substr(quoteSearch, 2) === quoteChar + quoteChar ||
                                   input.substr(quoteSearch, 4) === quoteChar + quoteChar + delim + quoteChar ||
                                   input.substr(quoteSearch, 2 + newlineLen) === quoteChar + quoteChar + newline
                               )) {
                                continue;
                            }
                            quoteSearch++;
                        ...
                        if(config.badQuotes &&
                           input[quoteSearch - 1] !== delim &&
                           input[quoteSearch + spacesBetweenQuoteAndDelimiter + 1] !== delim &&
                           input.substr(quoteSearch + 1 + spacesBetweenQuoteAndDelimiter, newlineLen) !== newline) {
                            continue;
                        }
