...
class ServerReportHelper {
    ...
    protected function getStatus(): array {
        $autoBackupStatus = $this->config->getAppValueInt('auto-backup/status');
    ...
    protected function getApps(): array {
        ...
        $data     = [];
        foreach(['guests', 'occweb', 'theming', 'passman', 'unsplash', 'impersonate', 'passwords_handbook', 'user_migration'] as $app) {
            $data[ $app ] = [
                'installed' => false,
                'enabled'   => false
            ];
