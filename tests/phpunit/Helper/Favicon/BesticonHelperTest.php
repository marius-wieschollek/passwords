...
class BesticonHelperTest extends TestCase {
    ...
    /**
     * @var MockObject|\OCA\Passwords\Helper\Image\ImagickHelper
     */
    private $imageHelper;
    ...
    protected function setUp(): void {
        ...
        $this->imageHelper           = $this->createMock(\OCA\Passwords\Helper\Image\ImagickHelper::class);
    ...
    public function testCallsWithInstance1Api() {
        ...
        $this->configurationService->method('getAppValue')->willReturnMap(
            [
                ...
                [$this->besticonHelper::BESTICON_COUNTER_KEY, '0:0', Application::APP_NAME, '0:0'],
            ]
        );
        $this->httpRequest->method('sendWithRetry')->willReturn($faviconData);
        $this->imageHelper->method('supportsImage')->with($faviconData)->willReturn(true);
    ...
    public function testWeekAndCounterAreSet() {
        ...
        $this->configurationService->method('getAppValue')->willReturnMap(
            [
                [$this->besticonHelper::BESTICON_CONFIG_KEY, '', Application::APP_NAME, ''],
                [$this->besticonHelper::BESTICON_COUNTER_KEY, '0:0', Application::APP_NAME, '10:5'],
            ]
        );
        $this->httpRequest->method('sendWithRetry')->willReturn($faviconData);
        $this->imageHelper->method('supportsImage')->with($faviconData)->willReturn(true);

        $this->configurationService->expects($this->once())->method('setAppValue')->with($this->besticonHelper::BESTICON_COUNTER_KEY, '10:6');
    ...
    public function testWeekAndCounterAreReset() {
        ...
        $this->configurationService->expects($this->once())->method('setAppValue')->with($this->besticonHelper::BESTICON_COUNTER_KEY, '10:1');
    ...
    public function testNotificationSentIfLimitReached() {
        ...
        $this->configurationService->method('getAppValue')->willReturnMap(
            [
                [$this->besticonHelper::BESTICON_CONFIG_KEY, '', Application::APP_NAME, ''],
                [$this->besticonHelper::BESTICON_COUNTER_KEY, '0:0', Application::APP_NAME, '10:'.($limit - 1)],
            ]
        );
        $this->httpRequest->method('sendWithRetry')->willReturn($faviconData);
        $this->imageHelper->method('supportsImage')->with($faviconData)->willReturn(true);
        ...
        $this->configurationService->expects($this->once())->method('setAppValue')->with($this->besticonHelper::BESTICON_COUNTER_KEY, '10:'.$limit);
